...
pandarallel.initialize(nb_workers=32, progress_bar=True)  # tune nb_workers (e.g. 8 or os.cpu_count()-1)
...
def convert_file(infile, out_csv, chunksize=1_000_000):
    ...
    reader = pd.read_csv(infile, sep=r'\s+', names=['smiles','zinc_id'], header=None, chunksize=chunksize, iterator=True)
    first_write = True
